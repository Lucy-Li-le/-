...
- Render 服务仅允许访问首页、CSS、JS 和两张 SVG；源码、`.env`、`.git` 不对外提供。
- 输入最多 2000 字，请求体最多 16 KiB；AI 请求最多同时 2 个、每进程每分钟最多 10 个。该限制是演示级全局防护，重启会重置，多实例不共享，不能替代生产级鉴权及持久化配额。
- 未配置 Key 时明确标注演示模式，摘要只摘录用户输入；其他 AI 错误显示失败。
- 不记录 Dify 原始错误体或异常字符串。已扫描当前可达 Git 历史的 18 个文件对象，未命中常见 Dify/OpenAI Key 或私钥头；这不是完整泄露审计。
- 自动检查通过：静态资源白名单、敏感路径 GET/HEAD 拒绝、错误 JSON、请求体/输入长度限制、未配置 Key、Dify text/result 解析和上游错误隐藏。
- JavaScript 模拟测试通过：保守演示内容、OKX 优先选择、十进制链 ID、本地存储失败后保留已确认交易结果。模拟测试不代表真实钱包验收。
- RPC `eth_getCode` 确认 `0x58BD27398C251Dea02Be46b5752D3502D62AEe7A` 存在 1248 字节代码；未核对部署回执和源码一致性。
- 待完成：比赛期间工作流实现情况确认、新 Key 配置、真实 Dify 调用、公网部署、浏览器视觉验收、真实上链、视频及 MOJO 提交。
